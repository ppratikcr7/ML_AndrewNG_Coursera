...
%Feed Forward propagation for gradients and cost function
X = [ones(m,1), X];
a1 = X;
z2 = Theta1*a1';
a2 = sigmoid(z2);
...
% make labels in NN output format of 10 values
yVec = zeros(num_labels, m);
for i = 1:m
    % place 1 for the column number = y_label, means the element belongs to that class
    yVec(y(i), i) = 1;
end


% Cost function without regularization
J_unreg = (1 / m) * sum(sum((-yVec) .* log(hThetaX) - (1 - yVec) .* log(1 - hThetaX)));

% Cost function with regularization
J = J_unreg + (lambda/(2*m))*(sum(sum(Theta1(:, 2:end).^2)) + sum(sum(Theta2(:, 2:end).^2)));

% Gradients for each set of theta: del/deltheta of J
